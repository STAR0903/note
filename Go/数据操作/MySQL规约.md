...
* 字段允许适当冗余，以提高性能，但是必须考虑数据同步的情况。冗余字段应遵循：
  ...
  解读：冗余字段是数据库设计中，主动将原本仅存储在某一张表中的字段，重复存储到另一张 / 多张关联表中 的字段；它打破了数据库规范化设计（如第三范式）中 “消除数据重复” 的原则，核心目的是以额外的存储成本换取查询性能提升，是 “空间换时间” 的典型设计策略。
  ![1764081115696](image/MySQL规约/1764081115696.png)
...
* SQL 性能优化的目标：至少要达到 range 级别，要求是 ref 级别，如果可以是 consts 最好。
  ...
  ![1764149207516](image/MySQL规约/1764149207516.png)
* 建组合索引的时候，区分度最高的在最左边。
  正例：如果 where a =? and b =? ， a 列的几乎接近于唯一值，那么只需要单建 idx _ a 索引即可。
  **说明** ：存在非等号和等号混合判断条件时，在建索引时，请把等号条件的列前置。如：where a >? and b =? 那么即使 a 的区分度更高，也必须把 b 放在索引的最前列。
  解读：一旦索引中使用了范围查找（ 如 `>`,`<`,`BETWEEN` ），那么索引的 **有序性就会中断** ，后续的字段将无法再利用索引进行快速定位或排序。
...
* count(distinct col) 计算该列除 NULL 之外的不重复数量。注意 count(distinctcol 1, col 2 ) 如果其中一列全为 NULL ，那么即使另一列有不同的值，也返回为 0。
* 当某一列的值全是 NULL 时， count(col) 的返回结果为 0，但 sum(col) 的返回结果为NULL ，因此使用 sum() 时需注意 NPE 问题 ( 空指针异常 )。
...
[https://www.cnblogs.com/wangxin37/p/7084327.html](https://www.cnblogs.com/wangxin37/p/7084327.html)
