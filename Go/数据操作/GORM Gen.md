...
我们可以在 ./dal 下新建 `db.go`文件，保存如下初始化数据库连接的代码。
...
func main() {
	...
	// 也可以手动指定需要生成代码的数据表
	g.ApplyBasic(g.GenerateModel("book"))
...
func main() {
	...
	if err != nil {
		fmt.Printf("create book failed: %v", err)
		return
	}
	...
	info, err := query.Book.WithContext(context.Background()).
		...
		Update(query.Book.Price, 300)
	if err != nil {
		fmt.Printf("update book failed: %v", err)
		return
	}
	fmt.Printf("RowsAffected: %v\n", info.RowsAffected)

	// 查询
	b2, err := query.Book.WithContext(context.Background()).Where(query.Book.ID.Eq(1)).First()
	if err != nil {
		fmt.Printf("query book failed: %v", err)
		return
	}
	fmt.Printf("b2: %v\n", b2)

	// 删除
	info, err = query.Book.WithContext(context.Background()).Where(query.Book.ID.Eq(1)).Delete()
	if err != nil {
...
if err != nil {
	fmt.Printf("GetBooksByAuthor failed, err:%v\n", err)
	return
}
for i, b := range rets {
	fmt.Printf("%d:%v\n", i, b)
...
```go
// 使用自定义的FilterWithColumn方法
ret, err := query.Book.WithContext(context.Background()).FilterWithColumn("title", "《Go语言》")
fmt.Printf("ret: %v\n", ret)
fmt.Printf("err: %v\n", err)
```
