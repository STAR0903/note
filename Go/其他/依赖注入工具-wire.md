...
控制反转（Inversion of Control，缩写为IoC），是 **面向对象编程中的一种设计原则，可以用来减低计算机代码之间的耦合度** 。

其中最常见的方式叫做依赖注入（Dependency Injection，简称DI）。依赖注入是生成灵活和松散耦合代码的标准技术，通过明确地向组件提供它们所需要的所有依赖关系。
...
比如：Uber的[dig](https://github.com/uber-go/dig)和Facebook的[inject](https://github.com/facebookgo/inject)都使用反射来做运行时依赖注入。
...
func NewZ(ctx context.Context,y Y) (Z, error) {
	...
	return Z{Value: y.Value + 100}, nil
}
```
...
func initZ(ctx context.Context) (demo.Z, error) {
	// wire.Build(demo.NewZ, demo.NewX, demo.NewY)
...
可以使用提供的类型构造结构体。使用 `wire.Struct`函数构造一个结构体类型，并告诉注入器应该注入哪个字段。注入器将使用字段类型的提供程序填充每个字段。例如，给定以下提供者：
...
如果注入器返回的是 `*FooBar` 而不是 `FooBar`，那么对应的 `wire.go` 和生成的注入器将如下所示:
