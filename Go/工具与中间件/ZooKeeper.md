...
| 核心作用          | 扮演的角色           | 解决的主要问题                                           |
| ----------------- | -------------------- | -------------------------------------------------------- |
| 1. 配置管理       | 统一的配置中心       | 分布式系统配置分散、难以动态更新的问题。                 |
| 2. 命名服务与发现 | 服务的“电话簿”     | 客户端如何高效、动态地找到集群中的服务实例地址。         |
| 3. 分布式同步/锁  | 分布式“交通警察”   | 多个进程并发访问和修改共享数据时的数据一致性及竞态条件。 |
| 4. 集群管理与选举 | 集群的“健康监控器” | 集群状态维护、成员变动通知、自动 Master/Leader 选举。    |
...
Leader 是集群主节点，主要负责管理集群状态和接收用户的写请求；Follower 是从节点，主要负责集群选举投票和接收用户的读请求；Observer 的功能与 Follower 类似，只是没有投票权，主要用于分担 Follower 的读请求，降低集群的负载。
...
![1764525093467](image/ZooKeeper/1764525093467.png)
...
前面介绍了 ZooKeeper 的基本概念、角色、工作机制等内容，为了后续更好地学习 ZooKeeper 的原理，本节先给大家介绍一种分布式一致性协议——ZAB (ZooKeeper Atomic Broadcast) 即ZooKeeper 原子消息广播协议。

 ZAB 协议最初也只是为雅虎内部那些高吞吐量、低延迟的分布式系统场景设计的，它并不是一种通用的分布式一致性算法，而是专门为 ZooKeeper 设计的一种支持崩溃恢复的原子广播协议，实现了一种主备模式的系统架构来保持集群中各副本之间数据的一致性。
...
![1764525960153](image/ZooKeeper/1764525960153.png)
...
![1764526247492](image/ZooKeeper/1764526247492.png)
...
![1764526280677](image/ZooKeeper/1764526280677.png)
...
ZooKeeper 可用于统一命名服务、配置管理、集群管理、分布式通知协调、分布式锁等场景，在这些应用场景中， ZooKeeper 内部是如何做到分布式数据一致性的呢？
...
![1764526346966](image/ZooKeeper/1764526346966.png)
...
每一个 Znode 节点上都具有原子操作的特性，读操作将获取与节点相关的数据，写操作将替换节点上的数据，ZooKeeper 都会为每一个事务请求分配一个全局唯一的事务编号 Zxid，每一个 Zxid 就对应一次更新操作，并可以通过 Zxid 来识别出更新操作请求的全局顺序。
...
![1764526781480](image/ZooKeeper/1764526781480.png)
...
![1764527176782](image/ZooKeeper/1764527176782.png)
...
![1764527253484](image/ZooKeeper/1764527253484.png)
...
![1764527265439](image/ZooKeeper/1764527265439.png)
...
![1764527311885](image/ZooKeeper/1764527311885.png)
...
[一文看懂 ZooKeeper ，面试再也不用背八股 - Data跳动 - 博客园](https://www.cnblogs.com/datadance/p/16293445.html)
